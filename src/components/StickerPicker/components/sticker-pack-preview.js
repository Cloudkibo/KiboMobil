...
class StickerPackPreview extends Component {
  ...
  render () {
    ...
    return (
      <Block className='sticker-pack-preview'>
        <Block className='preview-header' style={style.header}>
          <Text h1 style={style.heading}>{preview.title}</Text>
          <Block onPress={closePreview} className='button-close'><IconClose color={colors.secondary} /></Block>
        </Block>
        <Block className='preview-body'>
          <Image style={{width: 50, height: 50}} source={{ uri: preview.preview_landscape.hdpi }} />
        </Block>
        <Block className='preview-footer'>
          <Block onPress={this.purchasePack} className='button-purchase'><IconPurchase color={colors.primary} /></Block>
        </Block>
      </Block>
    )
  }
